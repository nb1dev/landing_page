...
export const OutcomesComponent: React.FC<Props> = ({
  heading, subheading, gaugeScore, gaugeMax, gaugeLabel,
  deltaLabel, deltaFrom, builtInText, feltText, cards, footnote,
}) => {
  ...
  const syncArrows = () => {
    ...
    if (max <= DEAD) { setShowPrev(false); setShowNext(false); return }
  ...
  return (
    <>
      <style jsx>{`
        ...
        @media (max-width: 760px) { .rto-sec { padding: 72px 0; } }

        .rto-wrap { max-width: 1200px; margin: 0 auto; }
        .rto-pad { padding: 0 48px; }
        @media (max-width: 640px) { .rto-pad { padding: 0 24px; } }

        /* ── Head ── */
        .rto-head { text-align: center; max-width: 680px; margin: 0 auto; }
        .rto-head :global(h2) {
        ...
        .rto-head :global(h2 em) { font-style: normal; color: #0A8FB0; }
        .rto-sub {
          margin: 18px auto 0;
          font-size: 16.5px;
          font-weight: 300;
        ...
        .rto-gauge svg { width: 100%; height: auto; display: block; overflow: visible; }
        .rto-gauge .track { fill: none; stroke: #c9d2d4; stroke-width: 22; stroke-linecap: round; }
        .rto-gauge .val { fill: none; stroke: #0A8FB0; stroke-width: 22; stroke-linecap: round; }
        .rto-gauge-center {
          position: absolute;
        ...
        .rto-gscore .gu { font-size: 26px; font-weight: 600; color: #9aa7ad; letter-spacing: 0; }
        .rto-glabel {
          margin-top: 8px;
          font-size: 12px;
          font-weight: 600;
        ...
        .rto-chip {
          ...
          font-weight: 600;
          color: #3f9a82;
          background: rgba(63,154,130,0.10);
          border: 1px solid rgba(63,154,130,0.34);
          border-radius: 10px;
          padding: 7px 14px;
        }
        .rto-from { font-size: 14px; font-weight: 400; color: #6c7d88; }
        ...
        .rto-builtin :global(strong) { color: #11293b; font-weight: 600; }
        .rto-builtin :global(p) { margin: 0; }
        .rto-felt {
          text-align: center;
          max-width: 600px;
          margin: 50px auto 0;
        ...
        .rto-felt :global(strong) { color: #11293b; font-weight: 600; }
        .rto-felt :global(p) { margin: 0; }

        /* ── Carousel ── */
        .rto-carousel { position: relative; margin-top: 30px; }
        .rto-row {
          display: flex;
          gap: 20px;
          overflow-x: auto;
        ...
        .rto-row::-webkit-scrollbar { display: none; }
        @media (max-width: 900px) { .rto-row { padding: 6px 24px 24px; } }
        @media (max-width: 640px) { .rto-row { padding: 6px 24px 24px; } }
        ...
        @media (max-width: 900px) { .rto-card { flex-basis: calc((100% - 20px) / 2 - 14px); } }
        @media (max-width: 640px) { .rto-card { flex-basis: 80vw; } }

        .rto-inner { position: absolute; inset: 0; }
        .rto-face {
          position: absolute;
          inset: 0;
          border-radius: 18px;
        ...
        @media (prefers-reduced-motion: reduce) { .rto-face { transition: none; } }
        .rto-front { z-index: 2; opacity: 1; }
        .rto-back { z-index: 1; opacity: 0; pointer-events: none; }
        .rto-card.flipped .rto-front { opacity: 0; pointer-events: none; }
        .rto-card.flipped .rto-back { opacity: 1; pointer-events: auto; z-index: 4; }
        ...
        .rto-scrim-bot {
          ...
          background: linear-gradient(to top, rgba(8,16,22,.85) 0%, rgba(8,16,22,0) 46%);
        }
        .rto-scrim-top {
          position: absolute; inset: 0; z-index: 1; pointer-events: none;
          background: linear-gradient(to bottom, rgba(8,16,22,.35) 0%, rgba(8,16,22,0) 25%);
        }
        .rto-ctop {
        ...
        .rto-cat.back { text-shadow: none; color: rgba(255,255,255,.62); }
        ...
        .rto-flip {
          ...
          background: rgba(12,24,34,.5);
          backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px);
          color: #fff; cursor: pointer;
          display: flex; align-items: center; justify-content: center;
          padding: 0;
          transition: background .2s ease, transform .45s cubic-bezier(.4,.05,.2,1);
        }
        .rto-flip:hover { background: rgba(12,24,34,.78); }
        .rto-card.flipped .rto-flip { transform: rotate(135deg); }
        .rto-flip .ic {
          display: block; font-size: 21px; font-weight: 300; line-height: 1;
          transform: translateY(-1px);
        }
        @media (prefers-reduced-motion: reduce) { .rto-flip { transition: background .2s ease; } }

        /* Glass panel */
        .rto-glass {
          position: absolute; left: 14px; right: 14px; bottom: 14px; z-index: 3;
          background: rgba(12,24,34,.42);
        ...
        .rto-t { color: #fff; font-size: 16.5px; font-weight: 600; letter-spacing: -0.01em; line-height: 1.12; }
        .rto-chip2 {
          font-size: 10.5px; font-weight: 600; color: #0e2433;
          background: #7fe0c4; border-radius: 6px; padding: 3px 7px;
        ...
        .rto-nums .b { font-size: 13px; font-weight: 500; color: rgba(255,255,255,.5); }
        .rto-nums .ar { font-size: 11px; color: rgba(255,255,255,.4); }
        .rto-nums .n { font-size: 22px; font-weight: 600; color: #fff; }
        .rto-nums .u { font-size: 10px; color: rgba(255,255,255,.45); }

        /* Track bar */
        .rto-trk {
          position: relative; height: 4px; border-radius: 3px;
          background: rgba(255,255,255,.18); margin-top: 12px;
        }
        .rto-trk .seg {
          position: absolute; top: 0; height: 4px; border-radius: 3px;
          background: #13A6CC; opacity: .92;
        }
        .rto-trk .dot {
        ...
        .rto-trk .dot.b { background: rgba(255,255,255,.55); width: 7px; height: 7px; }
        .rto-trk .dot.n { background: #fff; box-shadow: 0 0 0 2px #13A6CC; }
        .rto-gfoot {
          margin-top: 9px; font-size: 10px; font-weight: 500;
          letter-spacing: 0.02em; color: rgba(255,255,255,.5);
        }
        ...
        .rto-back-body-wrap { margin: auto 0; }
        .rto-back-eyebrow {
          font-family: 'JetBrains Mono', monospace;
          font-size: 9.5px; font-weight: 600;
          letter-spacing: 0.18em; text-transform: uppercase;
          color: #13A6CC; margin-bottom: 12px;
        }
        .rto-back-body {
        ...
        .rto-arrow {
          ...
          color: #11293b; font-size: 18px; cursor: pointer;
          display: flex; align-items: center; justify-content: center;
          box-shadow: 0 8px 22px -10px rgba(17,41,59,.3);
        }
        .rto-arrow.prev { left: 10px; }
        .rto-arrow.next { right: 10px; }
        @media (max-width: 640px) { .rto-arrow { display: none; } }

        /* Footnote */
        .rto-foot { text-align: center; margin-top: 24px; }
        .rto-note {
          font-size: 13px; font-weight: 300; color: #9aa7ad;
      ...
      <section ref={secRef} className="rto-sec" data-screen-label="Outcomes">
        <div className="rto-wrap">
          ...
          {/* Gauge */}
          <div className={`rto-gauge r-up${visible ? ' in' : ''}`} style={{ transitionDelay: '0.08s' }}>
            <svg viewBox="0 0 340 196" role="img" aria-label={`Score ${gaugeScore} out of ${gaugeMax}`}>
            ...
            <div className="rto-gauge-center">
              <div className="rto-gscore">
                {gaugeScore}<span className="gu">/{gaugeMax}</span>
              </div>
          ...
          {(deltaLabel || deltaFrom) && (
            <div className={`rto-delta r-up${visible ? ' in' : ''}`} style={{ transitionDelay: '0.12s' }}>
          ...
          {/* Built-in */}
          {builtInText && (
            <div className={`rto-builtin rto-pad r-up${visible ? ' in' : ''}`} style={{ transitionDelay: '0.16s' }}>
          ...
          {/* Felt */}
          {feltText && (
            <div className={`rto-felt rto-pad r-up${visible ? ' in' : ''}`} style={{ transitionDelay: '0.2s' }}>
          ...
          {cards && cards.length > 0 && (
            <div className="rto-carousel">
              {showPrev && (
                <button className="rto-arrow prev" aria-label="Previous" onClick={() => scrollBy(-1)}>‹</button>
              )}
              {showNext && (
                <button className="rto-arrow next" aria-label="Next" onClick={() => scrollBy(1)}>›</button>
              )}
                  ...
                  return (
                    ...
                    >
                      <button
                        className="rto-flip"
                        type="button"
                        aria-label={card.flipAriaLabel || `See what's behind the ${card.category} number`}
                        ...
                        <div className="rto-face rto-front">
                          ...
                          <div className="rto-glass">
                            <div className="rto-grow">
                              ...
                              {card.deltaChip && <span className="rto-chip2">{card.deltaChip}</span>}
                            </div>
                            ...
                            <div className="rto-trk">
                              {card.trackSegLeft && card.trackSegRight && (
                                <span className="seg" style={{ left: card.trackSegLeft, right: card.trackSegRight }} />
                              )}
                              {card.trackDotBefore && <span className="dot b" style={{ left: card.trackDotBefore }} />}
                              {card.trackDotAfter && <span className="dot n" style={{ left: card.trackDotAfter }} />}
                            </div>
                            {card.trackFootnote && <div className="rto-gfoot">{card.trackFootnote}</div>}
                          </div>
                        </div>
                        ...
                        <div className="rto-face rto-back">
                          <div className="rto-back-face">
                            ...
                            <div className="rto-back-body-wrap">
                              {card.backEyebrow && <div className="rto-back-eyebrow">{card.backEyebrow}</div>}
